...
- **镜像滞后跳过**：目标版本已由官方 npm 发布、而某个 registry 尚未同步时，直接以 INFO 提示跳过该源，不再让 npm 以 `ETARGET` 报错收场。
- **残留备份清理**：npm 全局重装是「旧包改名成 `.<包名>-<8位随机>` 备份 → 装新包 → 删备份」；若此时 CLI 进程仍在运行，Windows 会拒绝删除（`npm warn cleanup ... EPERM`），备份会滞留（单份可达数百 MB）。每次检查会清点回收可确认属于 npm 的滞留备份，仍被进程占用的会保留并提示关闭对应进程后重跑；只匹配 npm 的暂存命名且要求同名正式目录仍存在，不会删除其他目录。
