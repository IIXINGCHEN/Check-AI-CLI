...
repair_tool_path() {
  local tool_id="$1" dirs=() dir idx candidates
  candidates="$(get_tool_candidate_dirs "$tool_id" || true)"
...
allow_remote_script_execution() {
  [ "${CHECK_AI_CLI_ALLOW_REMOTE_SCRIPT:-}" = "1" ]
}
...
confirm_remote_script_execution() {
  ...
  if [ "$AUTO_MODE" = "1" ]; then
    if allow_remote_script_execution; then
      log_warn "[SECURITY] Auto mode: remote script execution explicitly allowed for $tool_name from $url"
      return 0
    fi
...
update_factory() {
  ...
  if ! confirm_remote_script_execution "$url" "Factory CLI"; then
    log_warn "Remote script execution declined, trying npm fallback..."
  elif fetch_text "$url" | sh; then
    repair_tool_path factory >/dev/null 2>&1 || true
    return 0
  fi
