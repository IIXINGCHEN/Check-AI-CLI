...
assert_contains "$text" 'npm_registry_version_state' 'mirror sync-lag probe'
assert_contains "$text" 'has not published v$target yet; skipping this source' 'informational mirror sync-lag skip'
assert_contains "$text" 'invoke_npm_staging_cleanup' 'staging leftover cleanup'
assert_contains "$text" 'npm_staging_dirs' 'staging leftover detection'
